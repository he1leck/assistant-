...
// ====== Ключ API (нужен вне claude.ai) — Google Gemini, бесплатный лимит ======
function getApiKey() {
  return localStorage.getItem('assistant_gemini_key') || '';
}
...
function ensureApiKeyPrompt() {
  ...
  bar.innerHTML = 'Нужен бесплатный API-ключ Google Gemini, чтобы приложение отвечало. <a href="#" id="setkey" style="color:#4f6ef7">Ввести ключ</a>';
  document.getElementById('setkey').addEventListener('click', (e) => {
    e.preventDefault();
    const key = prompt('Вставь свой Gemini API-ключ (получить бесплатно на aistudio.google.com/apikey):');
    if (key) {
      localStorage.setItem('assistant_gemini_key', key.trim());
      ensureApiKeyPrompt();
    }
  });
}
...
// ====== Отправка в ИИ (Google Gemini API, бесплатный тариф) ======
async function askAI(text) {
  const key = getApiKey();
  if (!key) {
    ensureApiKeyPrompt();
    return 'Сначала добавь бесплатный API-ключ (см. сообщение выше), чтобы я мог отвечать.';
  }
  statusEl.textContent = 'думаю...';
  try {
    // Gemini использует roles 'user' и 'model' (не 'assistant'), формат contents/parts
    ...
    if (!reply) {
      if (data?.error?.message) return 'Ошибка Gemini: ' + data.error.message;
      return 'Извини, не получилось ответить.';
    }
    return reply;
  } catch (e) {
    statusEl.textContent = 'готов';
...
// ====== Фикс для iOS: поле ввода не уезжает под клавиатуру ======
